...
int main() {
    ...
    // V5.1: Output is Single Channel R (Rectified Thermal)
    // We do NOT blend with IR. We just project Thermal to IR view.
    std::vector<uint8_t> rectified_r_buffer(w * h);
    ...
    for (int y = 0; y < h; ++y) {
        ...
        uint8_t* row_out = rectified_r_buffer.data() + y * w;
        ...
        for (int x = 0; x < w; x += 2) {
            ...
            uint8_t rgb_th[3];
            // Sample Thermal (this returns RGB, but we only care about R if source is grayscale/heatmap)
            // Assuming we take R channel.
            sample_fixed(th_ptr, w_th, h_th, entry.th.u, entry.th.v, rgb_th);
            
            // Output Planar R (Pixel 1)
            row_out[x] = rgb_th[0]; // R
            
            // Output Planar R (Pixel 2)
            if (x+1 < w) {
                row_out[x+1] = rgb_th[0]; // Reuse same sample
            }
        }
    }
    ...
    std::cout << "Avg V5.1 Processing Time: " << (total_us / 1000.0) / iterations << " ms" << std::endl;

    // Verify Output (Save as Grayscale BMP)
    // Reconstruct BGR buffer for file write (Line-by-Line)
    std::vector<unsigned char> row_bgr(padded_row_size);
    for(int y=0; y<h; ++y) {
        for(int x=0; x<w; ++x) {
             uint8_t val = rectified_r_buffer[y*w + x];
             int idx = x * 3;
             row_bgr[idx+0] = val; // B
             row_bgr[idx+1] = val; // G
             row_bgr[idx+2] = val; // R
