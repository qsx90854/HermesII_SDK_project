...
void init_hardcoded_params(CameraMatrix& K_th, CameraMatrix& K_ir, Mat3x3& R, Vec3& T) {
    ...
    // T = {13.83f, 24.52f, 3.56f}; // Original
    // V7: Override T to Zero for Optical Center Alignment (Infinite Depth Assumption)
    T = {0.0f, 0.0f, 0.0f}; 
}
...
int main() {
    ...
    float dist = 1300.0f; // Irrelevant for pure rotation (T=0) but kept for math stability
    ...
    std::cout << "Building V7 (Pure Rotation / Optical Center Aligned) LUT..." << std::endl;
    ...
    for (int y = 0; y < lut_h; ++y) {
        ...
        // V7 Logic: T is {0,0,0} so Pc = P
        float Pc_y = Py - T.y; // Py
        float Pc_z = Pz - T.z; // Pz
    ...
    std::ofstream out_file("rectified_result_v7.bmp", std::ios::binary);
    int padded_row_size = (w * 3 + 3) & (~3);
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (; x < w; x+=2) {
             ...
             int idx = x * 3;
             row_out[idx+0] = (row_ir[idx+2] + rgb_th[2]) >> 1; 
             row_out[idx+1] = (row_ir[idx+1] + rgb_th[1]) >> 1; 
             row_out[idx+2] = (row_ir[idx+0] + rgb_th[0]) >> 1; 
             if (x+1 < w) {
                int idx2 = (x + 1) * 3;
    ...
    std::cout << "V7 Processing Time: " << std::chrono::duration_cast<std::chrono::milliseconds>(t_proc_end - t_proc_start).count() << " ms" << std::endl;
    return 0;
}
