...
int main() {
    ...
    // --- V8-4: Full Resolution LUT based on ROI ---
    ...
    // No alignment needed for Full Res
    // if (roi_x1 % 2 != 0) roi_x1--;
    // if (roi_y1 % 2 != 0) roi_y1--;
    
    // LUT Dimensions (Full Resolution)
    int lut_x_start = roi_x1;
    int lut_y_start = roi_y1;
    int lut_w = roi_x2 - roi_x1;
    int lut_h = roi_y2 - roi_y1;
    
    std::cout << "Building V8-4 Full Res LUT (" << lut_w << "x" << lut_h << ")... Memory: " 
    ...
    for (int y = 0; y < lut_h; ++y) 
    {
        ...
        // Full Res: real_y = lut_y_start + y (No * 2)
        int real_y = lut_y_start + y;
        ...
        for (int x = 0; x < lut_w; ++x) {
            // Full Res: real_x = lut_x_start + x (No * 2)
            int real_x = lut_x_start + x;
    ...
    for (int iter = 0; iter < iterations; ++iter) 
    {
        ...
        for (int y = roi_y1; y < roi_y2; ++y) {
            ...
            // Full Res: lut_y = y - lut_y_start (No shift)
            int lut_y = y - lut_y_start;
            if (lut_y < 0) continue; 
            ...
            // 2. Pure Thermal Calculation (ROI) - FULL RESOLUTION
            for (int x = roi_x1; x < roi_x2; ++x) { // x++
                // Full Res: lut_x = x - lut_x_start (No shift)
                int lut_x = x - lut_x_start;
                if (lut_x < 0) continue;
                ...
                // No optimization for neighbor pixel (Full processing)
            }
        }
    ...
    std::cout << "Avg Fusion Time (V8-4, Full Res, Single Channel R-Only, No Save): " << total_us / iterations << " us" << std::endl;
