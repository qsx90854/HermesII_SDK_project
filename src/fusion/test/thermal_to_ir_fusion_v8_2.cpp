...
int main() {
    ...
    std::cout << "Building V8-2 Partial LUT (" << lut_w << "x" << lut_h << ")... Memory: " 
    ...
    // Always get IR ptr for background
    ...
    for (int iter = 0; iter < iterations; ++iter) 
    {
        ...
        for (int y = roi_y1; y < roi_y2; ++y) {
            ...
            #if !ENABLE_R_ONLY
                 // const unsigned char* row_ir = ir_ptr + y * w * 3; // Already defined above
            #endif
            ...
            for (int x = roi_x1; x < roi_x2; x += 2) {
                ...
                // --- V8-2 Optimized Interpolation: X-Linear, Y-Nearest ---
                int u_fixed = entry.th.u;
                int v_fixed = entry.th.v;
                
                int y_int = (v_fixed + 8) >> 4; 
                int x_int = u_fixed >> 4;       
                int x_frac = u_fixed & 0xF;     
                
                uint8_t fused_val = 0; 
                
                if (x_int < 0 || x_int >= w_th - 1 || y_int < 0 || y_int >= h_th) {
                     fused_val = 0;
                } else {
                    int stride = w_th * 3;
                    (void)stride; // unused check
                    int idx = (y_int * w_th + x_int) * 3;
                    const unsigned char* p = th_ptr + idx;
                    ...
                    #if ENABLE_R_ONLY
                        // Calculate full RGB
                        int val_r = (p[0] * w0 + p[3] * w1) >> 4;
                        int val_g = (p[1] * w0 + p[4] * w1) >> 4;
                        int val_b = (p[2] * w0 + p[5] * w1) >> 4;
                        
                        // Write RGB (BMP is BGR)
                        int out_idx = x * 3;
                        row_out[out_idx+0] = (uint8_t)val_b; // B
                        row_out[out_idx+1] = (uint8_t)val_g; // G
                        row_out[out_idx+2] = (uint8_t)val_r; // R
                    #else
                        // Optimized: Calc R only for Fusion intensity
                        int val_r = (p[0] * w0 + p[3] * w1) >> 4;
                        fused_val = (uint8_t)val_r;
                    #endif
                }
                    ...
                    row_out[idx+0] = (ir_b * 3 + fused_val) >> 2; 
                    row_out[idx+1] = (ir_g * 3 + fused_val) >> 2; 
                    row_out[idx+2] = (ir_r * 3 + fused_val) >> 2; 
                    
                    if (x+1 < w) {
                        int idx2 = (x+1) * 3;
                        row_out[idx2+0] = (row_ir[idx2+0] * 3 + fused_val) >> 2; 
                        row_out[idx2+1] = (row_ir[idx2+1] * 3 + fused_val) >> 2; 
                        row_out[idx2+2] = (row_ir[idx2+2] * 3 + fused_val) >> 2; 
                    }
                #endif
            }
        }
    ...
    std::cout << "Avg Fusion Time (V8-2): " << total_us / iterations << " us" << std::endl;

    // 2. SAVING (Re-run Logic Line-by-Line)
    #if ENABLE_SAVE
    std::ofstream out_file("rectified_result_v8_2.bmp", std::ios::binary);
    ...
    std::vector<uint8_t> io_row_buffer(row_stride, 0); 
    ...
    for(int y=0; y<h; ++y) {
        ...
        if (y >= roi_y1 && y < roi_y2) 
        {
            ...
            if (lut_y >= 0 && lut_y < lut_h) {
                ...
                const unsigned char* row_ir = ir_ptr + y * w * 3;
                // Initialize with IR data for background
                ...
                for (int x = roi_x1; x < roi_x2; x += 2) {
                    ...
                    if (x_int >= 0 && x_int < w_th - 1 && y_int >= 0 && y_int < h_th) {
                    ...
                    #if ENABLE_R_ONLY
                        int idx = x * 3;
                        io_row_buffer[idx+0] = fused_val; 
                        io_row_buffer[idx+1] = fused_val;
                        io_row_buffer[idx+2] = fused_val;
                        
                        if (x+1 < w) {
                            io_row_buffer[idx+3] = fused_val;
                            io_row_buffer[idx+4] = fused_val;
                            io_row_buffer[idx+5] = fused_val;
                        }
                    #else
                        int idx = x * 3;
                        // Get IR
                        ...
                        int ir_g = row_ir[idx+1];
                        // Fusion Output
                        io_row_buffer[idx+0] = (row_ir[idx+0] * 3 + fused_val) >> 2; 
                        io_row_buffer[idx+1] = (row_ir[idx+1] * 3 + fused_val) >> 2; 
                        io_row_buffer[idx+2] = (row_ir[idx+2] * 3 + fused_val) >> 2; 
                        
                        if (x + 1 < w) {
                            io_row_buffer[idx+3] = (row_ir[idx+3] * 3 + fused_val) >> 2; 
                            io_row_buffer[idx+4] = (row_ir[idx+4] * 3 + fused_val) >> 2; 
                            io_row_buffer[idx+5] = (row_ir[idx+5] * 3 + fused_val) >> 2; 
                        }
                    #endif
                }
            }
