...
$(LIB_PC_TARGET): $(SDK_PC_OBJS) | $(BUILD_DIR)
	g++ -shared -o $@ $(SDK_PC_OBJS) -lpthread
...
# 編譯 RTSP Demo
$(RTSP_DEMO_TARGET): examples/rtsp_demo.cpp $(LIB_TARGET)
	$(CXX) $(CXXFLAGS) -o $@ $< -L$(BUILD_DIR) -lHermesII_sdk -Wl,-rpath,'$$ORIGIN/$(BUILD_DIR)' -lpthread
...
$(SDK_ERRORLOG_VERIFIER_TARGET): examples/sdk_errorlog_verifier.cpp $(LIB_PC_TARGET)
	g++ $(COMMON_FLAGS) $(PC_FLAGS) -o $@ $< -L$(BUILD_DIR) -lHermesII_sdk_pc -Wl,-rpath,'$$ORIGIN/$(BUILD_DIR)' -lpthread

$(SDK_GRAY_TEST_EDGE_TARGET): examples/sdk_gray_test.cpp $(LIB_TARGET)
	$(CXX) $(CXXFLAGS) -o $@ $< -L$(BUILD_DIR) -lHermesII_sdk -Wl,-rpath,'$$ORIGIN/$(BUILD_DIR)' -lpthread

$(SDK_GRAY_TEST_PC_TARGET): examples/sdk_gray_test.cpp $(LIB_PC_TARGET)
	g++ $(COMMON_FLAGS) $(PC_FLAGS) -o $@ $< -L$(BUILD_DIR) -lHermesII_sdk_pc -Wl,-rpath,'$$ORIGIN/$(BUILD_DIR)' -lpthread

# 編譯 Event Recording Replay Verifier (讀取 SDK 自身事件錄影, 復驗跌倒判斷)
$(EVENT_REPLAY_VERIFY_EDGE_TARGET): examples/event_replay_verify.cpp $(LIB_TARGET)
	$(CXX) $(CXXFLAGS) -o $@ $< -L$(BUILD_DIR) -lHermesII_sdk -Wl,-rpath,'$$ORIGIN/$(BUILD_DIR)' -lpthread

$(EVENT_REPLAY_VERIFY_PC_TARGET): examples/event_replay_verify.cpp $(LIB_PC_TARGET)
	g++ $(COMMON_FLAGS) $(PC_FLAGS) -o $@ $< -L$(BUILD_DIR) -lHermesII_sdk_pc -Wl,-rpath,'$$ORIGIN/$(BUILD_DIR)' -lpthread

# 編譯 .gray 跌倒事件計數工具
$(SDK_GRAY_COUNT_FALLS_EDGE_TARGET): examples/sdk_gray_count_falls.cpp $(LIB_TARGET)
	$(CXX) $(CXXFLAGS) -o $@ $< -L$(BUILD_DIR) -lHermesII_sdk -Wl,-rpath,'$$ORIGIN/$(BUILD_DIR)' -lpthread

$(SDK_GRAY_COUNT_FALLS_PC_TARGET): examples/sdk_gray_count_falls.cpp $(LIB_PC_TARGET)
	g++ $(COMMON_FLAGS) $(PC_FLAGS) -o $@ $< -L$(BUILD_DIR) -lHermesII_sdk_pc -Wl,-rpath,'$$ORIGIN/$(BUILD_DIR)' -lpthread

# 編譯 ini_read_test (純檔案讀取隔離測試, 完全不連結 SDK)
$(INI_READ_TEST_EDGE_TARGET): examples/ini_read_test.cpp
	$(CXX) $(CXXFLAGS) -o $@ $<

$(INI_READ_TEST_PC_TARGET): examples/ini_read_test.cpp
	g++ $(COMMON_FLAGS) $(PC_FLAGS) -o $@ $<
...
# 清除規則
clean:
	rm -f $(SDK_OBJS) $(FALL_DEMO_TARGET) $(FALL_CALLBACK_TARGET) $(SDK_DEMO_CODE_V1_TARGET) $(SDK_DEMO_CODE_V2_TARGET) $(SDK_DEMO_TEST_ACCURACY_TARGET) $(STREAMING_DEMO_TARGET) $(RTSP_DEMO_TARGET) $(FALL_ACCURACY_TEST_TARGET) $(SDK_DEMO_BG_TEST_TARGET) $(SDK_ERRORLOG_VERIFIER_TARGET) $(SDK_GRAY_TEST_PC_TARGET) $(SDK_GRAY_TEST_EDGE_TARGET) $(EVENT_REPLAY_VERIFY_PC_TARGET) $(EVENT_REPLAY_VERIFY_EDGE_TARGET) $(SDK_GRAY_COUNT_FALLS_PC_TARGET) $(SDK_GRAY_COUNT_FALLS_EDGE_TARGET) $(INI_READ_TEST_PC_TARGET) $(INI_READ_TEST_EDGE_TARGET)
	rm -rf $(BUILD_DIR)
